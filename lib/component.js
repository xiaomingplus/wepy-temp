...
var _class = function () {
    ...
    _createClass(_class, [{
        ...
        value: function $init($wxpage, $root, $parent) {
            ...
            if (this.$props) {
                // generate mapping Props
                for (key in this.$props) {
                    for (binded in this.$props[key]) {
                        if (/\.sync$/.test(binded)) {
                            // sync goes to mapping
                        ...
                        if (binded) {
                            if ((typeof binded === 'undefined' ? 'undefined' : _typeof(binded)) === 'object') {
                                (function () {
                                    ...
                                    if ((typeof binddata === 'undefined' ? 'undefined' : _typeof(binddata)) === 'object' && typeof binddata !== null) {
        ...
        value: function $digest() {
            ...
            while (this.$$phase) {
                ...
                if (this.computed) {
                    for (k in this.computed) {
                        // If there are computed property, calculated every times
                        var _fn2 = this.computed[k],
                            val = _fn2.call(this);
                        if (!_util2.default.$isEqual(this[k], val)) {
                            // Value changed, then send to ReadyToSet
                ...
                for (k in originData) {
                    if (!_util2.default.$isEqual(this[k], originData[k])) {
                        // compare if new data is equal to original data
                        // data watch trigger
                        if (this.watch) {
                            if (this.watch[k]) {
